#include "graphics/BoundingVolume.hpp"
...
namespace jse {
	...
	const vec4& Frustum::getPlane(const int plane) const {
		return m_planes[plane];
	}

	BoundingVolume::TestResult Frustum::testIntersection(const vec3& point) const
	{
	...
	BoundingVolume::TestResult BoundingBox::testIntersection(const vec3& point) const
	{
		const vec3 max = maximum + position;
		const vec3 min = minimum + position;
	...
	{
		const vec3 min2 = box.minimum + box.position;
		const vec3 max1 = maximum + position;
